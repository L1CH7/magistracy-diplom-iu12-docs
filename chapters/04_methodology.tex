...
\begin{itemize}
\item \textbf{Short-Range Profile}: Cluster 2, mean distance 12.3\,km. Graph traversal overhead is small. Initialization and fast-clear costs dominate the execution profile.
\item \textbf{Medium-Range Profile}: Cluster 0, mean distance 28.4\,km. Traversal cost and queue operations exhibit equal latency footprints. 
\item \textbf{Long-Range Profile}: Cluster 1, mean distance 43.7\,km. The priority queue hot loop determines execution time.
\end{itemize}
This classification allows us to observe how priority queue performance scales from short-range queries to long-range paths.
