...
\begin{code}{overpass-ql-impl}{Text}{Запрос Overpass QL для фильтрации дорожного графа}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Наземный слой} -- объекты нулевого уровня, для которых запускается ресурсоемкий алгоритм поиска пересечений (узлования) для формирования перекрестков.
...
\begin{enumerate}
    ...
    \item \textbf{Итеративная обработка} -- тайлы обрабатываются последовательно, что удерживает потребление памяти в пределах 370 МБ (табл. \ref{tab:grid-performance}).
...
cur.execute("SET max_parallel_workers_per_gather = 12") -- Адаптируется под CPU
\end{code}

Сравнительная оценка производительности предложенного алгоритма секционирования и стандартного подхода к построению графа представлена в таблице \ref{tab:grid-performance}. Результаты испытаний подтверждают, что итеративная обработка позволяет удерживать потребление оперативной памяти на константном уровне, предотвращая аварийное завершение процесса.
